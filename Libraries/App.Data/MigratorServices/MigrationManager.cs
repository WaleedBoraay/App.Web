...
namespace App.Data.MigratorServices
{
    ...
    public partial class MigrationManager : IMigrationManager
    {
        ...
        protected readonly IVersionLoader _versionLoader;
            ...
            MigrationProcessType migrationProcessType = MigrationProcessType.NoMatter, bool isApplied = false, bool isSchemaMigration = false)
        {
            ...
            var migrations = _filteringMigrationSource
                .GetMigrations(t =>
                {
                    ...
                    if (isApplied == !_versionLoader.VersionInfo.HasAppliedMigration(migrationAttribute.Version))
                        return false;
        ...
        /// <summary>
        /// Executes an Up for all unapplied Update migrations
        /// </summary>
        /// <param name="assembly">Assembly to find migrations</param>
        public virtual void ApplyUpUpdateMigrations(Assembly assembly)
        {
            ArgumentNullException.ThrowIfNull(assembly);

            foreach (var migrationInfo in GetMigrations(assembly, false, MigrationProcessType.Update).OrderBy(m => m.Version))
        ...
        {
            // Use the _versionLoader field instead of _migrationRunner.VersionLoader
            var pendingMigrations = _migrationRunner.MigrationLoader.LoadMigrations()
                .Where(m => !_versionLoader.VersionInfo.HasAppliedMigration(m.Key))
                .ToList();
